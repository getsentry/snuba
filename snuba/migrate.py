...
def run(conn, SCHEMA):
    ...
    get_schema = lambda: {
        ...
        for column_name, column_type, default_type, default_expr in conn.execute(
            "DESCRIBE TABLE %s" % clickhouse_table
        )
    }
    ...
    for column_name, column_type in local_schema.items():
        if column_name not in SCHEMA.ALL_COLUMNS:
            logger.warn(
                "Column '%s' exists in local ClickHouse but not in schema!", column_name
            )
            continue
