...
def _time_split(query_func, dataset: Dataset, request: Request, *args, **kwargs):
    """
    If a query is:
...
def _col_split(query_func, dataset, request: Request, *args, **kwargs):
    """
...
def split_query(
    ...
    if common_conditions:
        ...
        if (
            ...
            return _col_split(query_func, dataset, request, *args, **kwargs)
        elif orderby[:1] == ['-timestamp'] and remaining_offset < 1000:
            return _time_split(query_func, dataset, request, *args, **kwargs)
