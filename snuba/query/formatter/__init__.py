from typing import Mapping, Any, Union, Sequence, Optional, List, MutableMapping
...
from snuba.query.data_source.join import JoinVisitor, JoinClause, IndividualNode
from snuba.query.data_source.simple import Entity

TExpression = Union[str, Mapping[str, Any], Sequence[Any]]


def format_query(query: Union[LogicalQuery, CompositeQuery[Entity]]) -> TExpression:
    return {
...
def _format_query_body(query: Query) -> Mapping[str, Any]:
    ...
    formatted = {
        ...
        "ORDERBY": [
            [e.expression.accept(expression_formatter), str(e.direction)]
            for e in query.get_orderby_from_ast()
        ],
    }
    array_join = query.get_arrayjoin_from_ast()
    if array_join:
        formatted["ARRAY JOIN"] = array_join.accept(expression_formatter)
