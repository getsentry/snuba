...
class ParsePopulateResolveMQL(
    ...
    def _process_data(
        ...
        with sentry_sdk.start_span(op="parser", name="parse_mql_query_initial"):
            query = parse_mql_query_body(mql_str, dataset)

        with sentry_sdk.start_span(op="parser", name="populate_query_from_mql_context"):
            query, mql_context = populate_query_from_mql_context(query, mql_context_dict)

        with sentry_sdk.start_span(op="processor", name="resolve_indexer_mappings"):
...
class PostProcessAndValidateMQLQuery(
    ...
    def _process_data(
        ...
        with sentry_sdk.start_span(op="processor", name="filter_in_select_optimize"):
            if settings is None:
