    ...
    def execute(self, request: Request, runner: SingleQueryRunner) -> RawQueryResult:
        ...
        if common_conditions:
            ...
            if (
                ...
                return col_split(request, column_split_spec, runner)
            elif orderby[:1] == ["-timestamp"] and remaining_offset < 1000:
                return time_split(request, runner)

        return self.__default_strategy.execute(request, runner)


def time_split(request: Request, runner: SingleQueryRunner) -> RawQueryResult:
    """
    If a query is:
...
def col_split(
    request: Request, column_split_spec: ColumnSplitSpec, runner: SingleQueryRunner
):
    """
    ...
    if result.result["data"]:
        ...
        event_ids = list(
            set([event[column_split_spec.id_column] for event in result.result["data"]])
        )
