...
def execute_query_with_rate_limits(
    ...
    with RateLimitAggregator(
        ...
        if applied_quota_stats is not None:
            project_rate_limit_stats: Optional[RateLimitStats] = applied_quota_stats
            rate_limit_stats_container.add_stats(
                QUOTA_RATE_LIMIT_NAME, applied_quota_stats
            )
        else:
        ...
        if applied_quota_stats is not None and project_rate_limit_stats is not None:
            logger.warning(
                "Conflicting project rate limit configs",
                extra={
                    "quota": applied_quota_stats,
