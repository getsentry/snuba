...
# See EAP-377.
_LOCAL_JOIN_CLICKHOUSE_SETTINGS = {"distributed_product_mode": "local"}


def use_local_join_for_cross_item_queries() -> bool:
    """Whether cross-item queries should push the ``trace_id`` join down to the
    local storage nodes (see EAP-377).

    When enabled, the ``trace_id IN (subquery)`` condition is emitted against the
    bare ``trace_id`` UUID column (rather than the dash-stripped
    ``replaceAll(toString(trace_id), '-', '')`` form) so the trace_id bloom-filter
    index can be used, and ``distributed_product_mode='local'`` is set so the join
    runs locally on each shard. Gated behind a runtime config so it can be rolled
    out and rolled back without a deploy.
    """
    return bool(state.get_int_config("use_local_join_for_cross_item_queries", default=0))


def local_join_clickhouse_settings() -> dict[str, str]:
    """ClickHouse settings to enable the local cross-item join, or an empty dict
    when the optimization is disabled."""
    if use_local_join_for_cross_item_queries():
        return dict(_LOCAL_JOIN_CLICKHOUSE_SETTINGS)
    return {}
...
def trace_id_in_subquery_condition(trace_ids_sql: str) -> Expression:
    ...
    When the local-join optimization is enabled, the condition is emitted as raw
    SQL referencing the bare ``trace_id`` UUID column. This keeps the column out of
    the ``UUIDColumnProcessor`` (so it is not wrapped in
    ``replaceAll(toString(trace_id), '-', '')``, which would defeat the bloom-filter
    index) and out of the ``PrewhereProcessor`` (so it stays in ``WHERE`` rather
    than ``PREWHERE``, which is incompatible with ``distributed_product_mode='local'``
    due to a ClickHouse bug). The subquery itself selects the bare ``trace_id`` (see
...
def get_trace_ids_sql_for_cross_item_query(
    ...
    # When local joins are enabled, select the bare ``trace_id`` UUID instead of the
    # dash-stripped ``replaceAll(toString(trace_id), '-', '')`` form so the outer
    # ``trace_id IN (subquery)`` comparison is UUID-to-UUID and can use the bloom-filter
    # index. The SELECT and GROUP BY expressions must match, so both use the bare column.
