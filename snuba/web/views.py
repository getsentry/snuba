...
RespTuple = tuple[str, int, dict[Any, Any]]
...
def handle_invalid_json(exception: InvalidJsonRequestException) -> Response:
    ...
    data: Mapping[str, Mapping[str, str | Sequence[str]]]
...
def unqualified_query_view(*, timer: Timer) -> Response | str | WerkzeugResponse:
...
@util.time_request("query")
def snql_dataset_query_view(*, dataset: Dataset, timer: Timer) -> Response | str:
    if http_request.method == "GET":
...
def mql_dataset_query_view(*, dataset: Dataset, timer: Timer) -> Response | str:
    if http_request.method == "POST":
        dataset_name = get_dataset_name(dataset)
        body = parse_request_body(http_request)
        _trace_transaction(dataset_name)
...
if application.debug or application.testing:
    ...
    def eventstream(*, entity: Entity) -> RespTuple:
        ...
        version = record[0]
        if version != 2:
            raise RuntimeError(f"Unsupported protocol version: {record}")
        ...
        try:
            ...
            raise InternalServerError(str(e), original_exception=e) from e
    ...
    def error() -> RespTuple:
        1 / 0  # noqa: B018 intentionally raises ZeroDivisionError to exercise the error path
