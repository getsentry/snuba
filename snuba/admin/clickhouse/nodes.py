...
from snuba.clusters.cluster import DEFAULT_CLICKHOUSE_HTTP_PORT, UndefinedClickhouseCluster
...
def _get_nodes(storage_key: StorageKey, local: bool = True) -> Sequence[Node]:
    try:
        ...
        if storage_key == StorageKey.DISCOVER:
            ...
            return []
        return [
            {"host": node.host_name, "port": node.port or DEFAULT_CLICKHOUSE_HTTP_PORT}
