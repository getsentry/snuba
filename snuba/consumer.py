...
def get_hash_state_map(connection, keys):
    results = {}
    for chunk in chunked(keys, ((2 ** 15) - 1) // 2):  # parameter list max length is int16, each key contains two params
        with connection.cursor() as cursor:
            cursor.execute(
                u'SELECT project_id, hash, group_id, deleted_at FROM grouphash_state WHERE (project_id, hash) IN (VALUES {conditions})'.format(
                    conditions=u', '.join([u'(%s, %s)'] * len(chunk)),
                ),
                list(itertools.chain.from_iterable(chunk)),
            )
            for row in cursor.fetchall():
                results[tuple(row[:2])] = tuple(row[2:])
    return results
...
def repair_batch_replacements(epoch, records):
    ...
    while i < len(records):
        ...
        if datetime.strptime(payload[2]['datetime'], settings.PAYLOAD_DATETIME_FORMAT).replace(tzinfo=pytz.utc) < epoch:
            del records[i]
        else:
            i = i + 1
...
class ConsumerWorker(AbstractBatchWorker):
    def __init__(self, clickhouse, dist_table_name, producer, replacements_topic, metrics=None):
        ...
        self.backfill_export_connection.autocommit = True

        self.backfill_export_epoch = datetime(2018, 10, 25, 17, 27, 7, 6685, tzinfo=pytz.utc)
    ...
    def flush_batch(self, batch):
        ...
        if inserts:
            original_insert_count = len(inserts)
            updated_insert_count = repair_batch_inserts(self.backfill_export_connection, self.backfill_export_epoch, inserts)
            logger.debug('Repaired %s insert records, %s remaining after filtering (%s updated.)', original_insert_count, len(inserts), updated_insert_count)
        ...
        if replacements:
            original_replacement_count = len(replacements)
            repair_batch_replacements(self.backfill_export_epoch, replacements)
            logger.debug('Repaired %s replacement records, %s remaining after filtering.', original_replacement_count, len(replacements))
