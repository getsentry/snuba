...
@click.command()
@click.option(
    '--clickhouse-server', multiple=True, help='Clickhouse server to cleanup.'
)
@click.option(
    '--dry-run',
    type=bool,
...
@click.option('--table', help='Name of the table to target.')
...
def cleanup(clickhouse_server, dry_run, database, table, log_level):
    ...
    for server in clickhouse_server:
        clickhouse = ClickhousePool(
            server.split(':')[0], port=int(server.split(':')[1])
        )
