...
@click.command()
@click.option(
    '--raw-events-topic', default='events', help='Topic to consume raw events from.'
)
@click.option(
    '--replacements-topic',
    default='event-replacements',
...
@click.option(
    '--dataset',
    default='events',
    type=click.Choice(settings.DATASETS.keys()),
...
def consumer(
    ...
    clickhouse = ClickhousePool(
        ...
        client_settings={'load_balancing': 'in_order', 'insert_distributed_sync': True},
        metrics=metrics,
    )
    ...
    # TODO figure out how many of these options are dataset
    # specific, and put them on the dataset object
