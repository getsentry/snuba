...
class Processor(object):
    ...
    def process_replacement(self, type_, message):
        ...
        to replace rows in the dataest with new ones
        """
        raise NotImplementedError
...
class EventsProcessor(Processor):
    ...
    def process_replacement(self, type_, message):
        if type_ == 'end_delete_groups':
            return self.process_delete_groups(message)
        elif type_ == 'end_merge':
            return self.process_merge(message)
        elif type_ == 'end_unmerge':
            return self.process_unmerge(message)
        elif type_ == 'end_delete_tag':
            return self.process_delete_tag(message)
