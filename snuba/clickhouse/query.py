...
class ClickhouseQuery:
    ...
    def __init__(self,
        ...
        aggregate_exprs = [util.column_expr(dataset, col, body, parsing_context, alias, agg) for (agg, col, alias) in query.get_aggregations()]
        groupby = util.to_list(query.get_groupby())
        group_exprs = [util.column_expr(dataset, gb, body, parsing_context) for gb in groupby]
        column_names = query.get_selected_columns() or []
        selected_cols = [util.column_expr(dataset, util.tuplify(colname), body, parsing_context) for colname in column_names]
        select_clause = u'SELECT {}'.format(', '.join(group_exprs + aggregate_exprs + selected_cols))

        from_clause = u'FROM {}'.format(source)

        if request.query.get_final():
            from_clause = u'{} FINAL'.format(from_clause)

        if query.get_sample():
            sample_rate = query.get_sample()
        elif request.settings.get_turbo():
        ...
        if query.get_conditions():
            where_clause = u'WHERE {}'.format(util.conditions_expr(dataset, query.get_conditions(), body, parsing_context))

        prewhere_clause = ''
        if prewhere_conditions:
            prewhere_clause = u'PREWHERE {}'.format(util.conditions_expr(dataset, prewhere_conditions, body, parsing_context))

        group_clause = ''
        if groupby:
            group_clause = 'GROUP BY ({})'.format(', '.join(util.column_expr(dataset, gb, body, parsing_context) for gb in groupby))
            if query.has_totals():
        ...
        if having_conditions:
            ...
            having_clause = u'HAVING {}'.format(util.conditions_expr(dataset, having_conditions, body, parsing_context))

        order_clause = ''
        if query.get_orderby():
            orderby = [util.column_expr(dataset, util.tuplify(ob), body, parsing_context) for ob in util.to_list(query.get_orderby())]
