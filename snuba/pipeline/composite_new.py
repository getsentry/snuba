...
class QueryPlannerNew(ABC, Generic[TPlan]):
    """
...
class QueryExecutionPipelineNew(ABC):
    ...
    @abstractmethod
    def create_plan(self) -> CompositeQueryPlan:
        raise NotImplementedError

    @abstractmethod
    def execute(self, plan: CompositeQueryPlan) -> QueryResult:
        raise NotImplementedError


class CompositeQueryPlannerNew(QueryPlannerNew[CompositeQueryPlan]):
    """
...
def _plan_composite_query(
    ...
    planned_data_source = CompositeDataSourcePlannerNew(settings).visit(
    ...
    return CompositeQueryPlan(
        ...
        execution_strategy=CompositeExecutionStrategyNew(
...
class JoinDataSourcePlanNew(NamedTuple):
    """
...
class JoinDataSourcePlanner(JoinVisitor[JoinDataSourcePlanNew, Entity]):
    """
    ...
    def visit_join_clause(self, node: JoinClause[Entity]) -> JoinDataSourcePlanNew:
...
class CompositeDataSourcePlanNew(NamedTuple):
    """
    Intermediate data structure maintained when visiting and
    ...
    ) -> CompositeDataSourcePlanNew:
        return CompositeDataSourcePlanNew(
...
class CompositeDataSourcePlannerNew(
    DataSourceVisitor[CompositeDataSourcePlanNew, Entity]
):
    """
    ...
    def _visit_simple_source(self, data_source: Entity) -> CompositeDataSourcePlanNew:
    ...
    def _visit_join(
        ...
        best_plans = select_best_plans(
            data_source.accept(JoinPlansRankerNew(self.__settings))
        )
    ...
    def _visit_simple_query(
        ...
        return CompositeDataSourcePlanNew.from_simple_query_plan(
    ...
    def _visit_composite_query(
        ...
        return CompositeDataSourcePlanNew.from_composite_subquery(
            _plan_composite_query(data_source, self.__settings)
        )


class ProcessorsExecutorNew(DataSourceVisitor[None, Table], JoinVisitor[None, Table]):
    """
...
class CompositeExecutionStrategyNew(QueryPlanExecutionStrategy[CompositeQuery[Table]]):
    """
...
class CompositeExecutionPipelineNew(QueryExecutionPipelineNew):
    """
    ...
    def create_plan(self) -> CompositeQueryPlan:
        plan = CompositeQueryPlannerNew(self.__query, self.__settings).build_best_plan()
        ...
        query = plan.execution_strategy.apply_processors()
        plan.query = query
        return plan

    def execute(self, plan: CompositeQueryPlan, runner: QueryRunner) -> QueryResult:
        return plan.execution_strategy.execute(plan.query, self.__settings, runner)
