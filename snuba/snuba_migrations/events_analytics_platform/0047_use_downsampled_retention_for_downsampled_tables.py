...
def generate_new_materialized_view_expression(sampling_weight: int) -> str:
    column_names_str = ", ".join(
        [
            c.name
            for c in columns
            if c.name != "sampling_weight"
            and c.name != "sampling_factor"
            and c.name != "retention_days"
        ]
    )
    return f"SELECT {column_names_str}, downsampled_retention_days AS retention_days, sampling_weight * {sampling_weight} AS sampling_weight, sampling_factor / {sampling_weight} AS sampling_factor FROM eap_items_1_local WHERE (cityHash64(item_id + {sampling_weight})  % {sampling_weight}) = 0"
