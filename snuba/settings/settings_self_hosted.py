...
VALID_RETENTION_DAYS = {int(env("SENTRY_EVENT_RETENTION_DAYS", 90)), 30, 60}
...
API_WORKERS_LIFETIME = (
    int(_workers_lifetime) if (_workers_lifetime := env("SNUBA_API_WORKERS_LIFETIME")) else None
)
API_WORKERS_MAX_RSS = (
    int(_workers_max_rss) if (_workers_max_rss := env("SNUBA_API_WORKERS_MAX_RSS")) else None
)
