...
def test_tracing_summary() -> None:
    ...
    assert (
        dist_summary.execute_summaries is not None
        and dist_summary.execute_summaries[0]
        == ExecuteSummary(
            rows_read=20826,
    ...
    assert (
        dist_summary.sorting_summaries is not None
        and dist_summary.sorting_summaries[0]
        == SortingSummary(
    ...
    assert (
        oneone_summary.execute_summaries is not None
        and oneone_summary.execute_summaries[0]
        == ExecuteSummary(
            rows_read=17264,
    ...
    assert (
        oneone_summary.select_summaries is not None
        and oneone_summary.select_summaries[0]
        == SelectSummary(
    ...
    assert (
        onetwo_summary.execute_summaries is not None
        and onetwo_summary.execute_summaries[0]
        == ExecuteSummary(
            rows_read=562,
    ...
    assert (
        onetwo_summary.select_summaries is not None
        and onetwo_summary.select_summaries[0]
        == SelectSummary(
    ...
    assert (
        onetwo_summary.index_summaries is not None
        and onetwo_summary.index_summaries[0]
        == IndexSummary(
            table_name="default.transactions_local",
    ...
    assert (
        onetwo_summary.stream_summaries is not None
        and onetwo_summary.stream_summaries[0]
        == StreamSummary(
            table_name="default.transactions_local", approximate_rows=86299, streams=4
        )
    )
