...
def test_tracing_summary() -> None:
    ...
    assert dist_summary.execute_summaries[0] == ExecuteSummary(
        rows_read=20826,
    ...
    assert dist_summary.sorting_summaries[0] == SortingSummary(
    ...
    assert oneone_summary.execute_summaries[0] == ExecuteSummary(
        rows_read=17264,
    ...
    assert oneone_summary.select_summaries[0] == SelectSummary(
    ...
    assert oneone_summary.aggregation_summaries[0] == AggregationSummary(
    ...
    assert onetwo_summary.execute_summaries[0] == ExecuteSummary(
        rows_read=562,
    ...
    assert onetwo_summary.select_summaries[0] == SelectSummary(
    ...
    assert onetwo_summary.index_summaries[0] == IndexSummary(
        table_name="default.transactions_local",
        index_name="bf_trace_id",
        dropped_granules=18,
        total_granules=19,
    )
    assert onetwo_summary.aggregation_summaries[0] == AggregationSummary(
        transform="AggregatingTransform",
        before_row_count=1,
        after_row_count=1,
        memory_size="17.00 B",
    ...
    assert onetwo_summary.stream_summaries[0] == StreamSummary(
