...
DISTS_ENTITY_KEY = EntityKey("metrics_counters")
DISTS_STORAGE_KEY = StorageKey("metrics_counters")
DISTS_STORAGE_SET_KEY = StorageSetKey("metrics")
...
...
"""
MOCK_LOGICAL_PART_MAPPING = {"metrics": {x: x % 2 for x in range(0, 256)}}
...
@patch("snuba.settings.SLICED_STORAGE_SETS", {"metrics": 2})
...
def test_column_based_partition_selector(
    ...
    if set_override:
        ...
        override = {MEGA_CLUSTER_RUNTIME_CONFIG_PREFIX: {"metrics": f"[{logical_partition}]"}}
    query = LogicalQuery(
        QueryEntity(
            DISTS_ENTITY_KEY,
