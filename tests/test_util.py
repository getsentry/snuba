...
class TestUtil(BaseTest):
    def test_column_expr(self):
        ...
        assert (
            column_expr(self.dataset, 'time', body.copy())
            == "(toDate(timestamp) AS time)"
        )

        assert (
            column_expr(self.dataset, 'col', body.copy(), aggregate='sum')
            == "(sum(col) AS col)"
        )

        assert (
            column_expr(self.dataset, None, body.copy(), alias='sum', aggregate='sum')
            == "sum"
        )  # This should probably be an error as its an aggregate with no column

        assert (
            column_expr(
                self.dataset, 'col', body.copy(), alias='summation', aggregate='sum'
            )
        ...
        assert (
            column_expr(
                self.dataset, '', body.copy(), alias='count', aggregate='count()'
            )
            == "(count() AS count)"
        )
        ...
        assert (
            column_expr(
                self.dataset, '', body.copy(), alias='aggregate', aggregate='count()'
            )
            == "(count() AS aggregate)"
        )
        ...
        assert (
            column_expr(self.dataset, 'sentry:release', body.copy())
            == '`sentry:release`'
        )
        ...
        assert (
            column_expr(self.dataset, '\'hello world\'', body.copy())
            == '\'hello world\''
        )
        ...
        assert (
            column_expr(
                self.dataset, tuplify(['concat', ['a', '\':\'', 'b']]), body.copy()
            )
            == 'concat(a, \':\', b)'
        )
        ...
        assert (
            column_expr(self.dataset, 'issue', group_id_body) == '(group_id AS issue)'
        )
    ...
    def test_alias_in_alias(self):
        ...
        assert (
            column_expr(self.dataset, 'tags_value', body)
    ...
    def test_conditions_expr(self):
        ...
        assert (
            conditions_expr(self.dataset, conditions, {})
            == '(a = 1 OR b = 2) AND c = 3'
        )
        ...
        conditions = [[['tags[foo]', '=', 1], ['b', '=', 2]]]
        column_expr(
            self.dataset, 'tags[foo]', reuse_body
        )  # Expand it once so the next time is aliased
        assert (
            conditions_expr(self.dataset, conditions, reuse_body)
            == '(`tags[foo]` = 1 OR b = 2)'
        )
    ...
    def test_complex_conditions_expr(self):
        ...
        assert (
            complex_column_expr(self.dataset, tuplify(['count', []]), body.copy())
            == 'count()'
        )
        assert (
            complex_column_expr(
                self.dataset, tuplify(['notEmpty', ['foo']]), body.copy()
            )
            == 'notEmpty(foo)'
        )
        assert (
        ...
        assert (
            complex_column_expr(
                self.dataset, tuplify(['foo', ['bar', ['qux'], 'baz']]), body.copy()
            )
            == 'foo(bar(qux), baz)'
        )
        assert (
            complex_column_expr(self.dataset, tuplify(['foo', [], 'a']), body.copy())
            == '(foo() AS a)'
        )
        assert (
            complex_column_expr(
                self.dataset, tuplify(['foo', ['b', 'c'], 'd']), body.copy()
            )
            == '(foo(b, c) AS d)'
        )
        assert (
        ...
        assert (
            complex_column_expr(
                self.dataset, tuplify(['topK', [3], ['project_id']]), body.copy()
            )
            == 'topK(3)(project_id)'
        )
        assert (
            complex_column_expr(
                self.dataset, tuplify(['topK', [3], ['project_id'], 'baz']), body.copy()
            )
            == '(topK(3)(project_id) AS baz)'
        )

        assert (
            complex_column_expr(
                self.dataset, tuplify(['emptyIfNull', ['project_id']]), body.copy()
            )
            == 'ifNull(project_id, \'\')'
        )
        assert (
        ...
        assert (
            complex_column_expr(
                self.dataset, tuplify(['safe', ['fo`o', "'ba'r'"]]), body.copy()
            )
            == r"safe(`fo\`o`, 'ba\'r')"
        )
        ...
        with pytest.raises(AssertionError):
            assert complex_column_expr(
                self.dataset, tuplify([r"dang'erous", ['message', '`']]), body.copy()
            )
    ...
    def test_referenced_columns(self):
        ...
        assert all_referenced_columns(body) == set(
