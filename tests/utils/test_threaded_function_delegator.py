...
def test() -> None:
    ...
    def callback_func(primary: Tuple[str, int], other: List[Tuple[str, int]]) -> None:
        assert result_received.wait(timeout=5), "Timeout while waiting for the main thread."
        callback_done.set()
    ...
    assert callback_done.wait(timeout=5), "Timeout while waiting for the callback to complete"
    ...
    assert mock_callback.call_args == call(Result("one", 1, ANY), [Result("two", 2, ANY)])
