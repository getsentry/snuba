...
def test_basic_snql() -> None:
    request, result = parse_and_run_query(
        body={
            "query": "MATCH (events) SELECT COUNT() WHERE timestamp >= toDateTime('2024-07-17T21:04:34') AND timestamp < toDateTime('2024-07-17T21:10:34') AND project_id = 1",
    ...
    print(result.result)
    print(result.result["data"][0]["COUNT()"])
    assert False
