...
class TestLastSeen(BaseApiTest):
    ...
    # Most frequent first: the exact opposite of BY_RECENCY_DESC, so neither ordering can
    # be satisfied by accident by the other.
    ...
    def staggered_items(self) -> None:
        ...
        for name, hours_ago in self.OFFSETS.items():
            ...
            # the number of items drives its count. Unique filler attributes keep each item
            # a distinct key set; their names avoid the "ls_" substring the tests filter on.
            write_raw_unprocessed_events(
                items_storage,
                [
                    gen_item_message(
    ...
    def test_recency_ordering_is_honoured(self) -> None:
        """Most recently used attributes first, with last_seen populated."""
