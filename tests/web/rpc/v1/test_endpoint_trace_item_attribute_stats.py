...
def setup_teardown(clickhouse_db: None, redis_db: None) -> None:
    ...
    messages = [
        gen_message(
            ...
            tags={"low_cardinality": f"{i % 40}", "high_cardinality": f"{i % 2}"},
        )
        for i in range(120)
    ]
...
class TestTraceItemAttributesStats(BaseApiTest):
    def test_basic_data(self, setup_teardown: Any) -> None:
        ts = Timestamp(seconds=int(BASE_TIME.timestamp()))
        hour_ago = int((BASE_TIME - timedelta(hours=3)).timestamp())
        message = TraceItemAttributesStatsRequest(
            meta=RequestMeta(
                project_ids=[1, 2, 3],
                organization_id=1,
        ...
        response = self.app.post(
            "/rpc/EndpointTraceItemAttributeStats/v1", data=message.SerializeToString()
        )
