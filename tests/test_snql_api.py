...
class TestSnQLApi(BaseApiTest):
    ...
    def test_search_issues_slo(self) -> None:
        # query comparing a tag to a number should return 400
        response = self.post(
            "/search_issues/snql",
            data=json.dumps(
                {
                    "query": """MATCH (search_issues)
                    SELECT uniq(group_id) AS `total`
                    BY group_id
                    WHERE timestamp >= now() AND timestamp < now() - 1
                    AND project_id IN tuple(6620563) AND (isNull(tags[error.type]) = 1
                    ORDER BY sample ASC LIMIT 100 OFFSET 0 TOTALS True
                    """,
                    "turbo": False,
                    "consistent": False,
                    "debug": True,
        ...
        # querying a non-existent column should return 400
        response = self.post(
            "/search_issues/snql",
            data=json.dumps(
                {
                    "query": """MATCH (search_issues)
                SELECT uniq(group_id) AS `total`
                BY group_id WHERE timestamp >= now() - 1
                AND timestamp < now() AND project_id IN tuple(5627728)
                AND location = 'this is not a real column'
                ORDER BY last_seen DESC, group_id ASC LIMIT 37 OFFSET 0 TOTALS True""",
                    "turbo": False,
                    "consistent": False,
                    "debug": True,
                }
