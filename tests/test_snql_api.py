...
class TestSnQLApi(BaseApiTest):
    ...
    def test_transaction_alloc_policy(self) -> None:
        from snuba.query.allocation_policies import AllocationPolicy

        policy = AllocationPolicy.get_from_name(
            "ConcurrentRateLimitAllocationPolicy"
        ).from_kwargs(
            storage_key="transactions", required_tenant_types=["organization_id"]
        )
        policy.set_config_value("is_enforced", True)
        policy.set_config_value("concurrent_limit", 0)
        response = self.post(
            "/events/snql",
            data=json.dumps(
                {
