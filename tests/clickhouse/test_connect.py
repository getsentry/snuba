...
def test_execute_preserves_native_datetime_metadata() -> None:
    client = mock.Mock()
    client.raw_query.return_value = json.dumps(
        {
    ...
    assert result.meta == [("time", "DateTime('Universal')")]
    assert result.results == [(datetime(2023, 1, 2, 3, 4, 5),)]


def test_execute_handles_unrecognized_server_timezone() -> None:
    # clickhouse-connect sets server_tz=None for unrecognized server timezones.
    # DateTime meta must still render without crashing on .tzname().
    client = mock.Mock()
    client.raw_query.return_value = json.dumps(
        {
            "meta": [{"name": "time", "type": "DateTime"}],
    ...
    result = pool.execute("SELECT now()", with_column_types=True)

    assert result.meta == [("time", "DateTime('Universal')")]


def test_jsoncompact_datetime_utc_normalizes_to_universal() -> None:
    # JSONCompact embeds the timezone in the type string. DateTime('UTC') must
    # still normalize to DateTime('Universal'), matching bare DateTime + UTC
    # server_tz and the old native driver spelling.
    client = mock.Mock()
    client.raw_query.return_value = json.dumps(
        {
            "meta": [
                {"name": "utc", "type": "DateTime('UTC')"},
                {"name": "ny", "type": "DateTime('America/New_York')"},
            ],
            "data": [["2023-01-02 03:04:05", "2023-01-02 03:04:05"]],
        }
    ).encode()

    result = _make_pool(client).execute("SELECT utc, ny", with_column_types=True)

    assert result.meta == [
        ("utc", "DateTime('Universal')"),
