...
class TestRateLimit:
    ...
    def test_ratelimit_aggregator(self, rate_limit_shards: Any) -> None:
        ...
        with (
            ...
            RateLimitAggregator([rate_limit_params1, rate_limit_params2]),
        ):
            pass

        with (
            pytest.raises(RateLimitExceeded),
            RateLimitAggregator([rate_limit_params2, rate_limit_params1]),
        ):
            pass
    ...
    def test_concurrent_limit(self, rate_limit_shards: Any) -> None:
        ...
        with pytest.raises(RateLimitExceeded), rate_limit(rate_limit_params):
            pass
        ...
        with (
            RateLimitAggregator([rate_limit_params1]),
            RateLimitAggregator([rate_limit_params2]),
        ):
            pass
    ...
    def test_aggregator(self, rate_limit_shards: Any) -> None:
        ...
        with (
            pytest.raises(RateLimitExceeded),
            RateLimitAggregator([rate_limit_params_outer, rate_limit_params_inner]),
        ):
            pass
    ...
    def test_rate_limit_exceptions(self) -> None:
        ...
        bucket = f"{state.ratelimit_prefix}{params.bucket}"
        ...
        with pytest.raises(RateLimitExceeded), rate_limit(params):
            assert count() == 2
...
def test_rate_limit_failures(vals: tuple[int, int, int], rate_limit_shards: Any) -> None:
    ...
    with pytest.raises(RateLimitExceeded), RateLimitAggregator(params):
        pass

    now = time.time()
    for p in params:
        bucket = f"{state.ratelimit_prefix}{p.bucket}"
