...
class TestOptimize(BaseTest):
    def test(self):
        ...
        parts = optimize.get_partitions_to_optimize(self.clickhouse, self.database, table)
        assert parts == []
        ...
        assert list(optimize.get_partitions_to_optimize(
            self.clickhouse, self.database, table, before=base
        )) == [(a_month_earlier_monday, 90)]

        optimize.optimize_partitions(self.clickhouse, self.database, table, parts)
