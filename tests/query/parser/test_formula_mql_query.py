...
def test_formula_filters() -> None:
    ...
    expected2 = Query(
        from_clause=expected1.get_from_clause(),
        selected_columns=expected1.get_selected_columns(),
        groupby=expected1.get_groupby(),
        condition=binary_condition(
            "and",
            in_clause(tag_column("status_code"), [Literal(None, "200")]),
            binary_condition(
        ...
        order_by=expected1.get_orderby(),
        limit=expected1.get_limit(),
        offset=expected1.get_offset(),
    )
    ...
    query, _ = parse_mql_query(str(query_body), mql_context, generic_metrics)
    eq1, reason = query.equals(expected1)
    eq2, _ = query.equals(expected2)
    assert eq1 or eq2, reason
