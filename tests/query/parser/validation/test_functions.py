...
from snuba.query.mql.parser import (
    MQL_POST_PROCESSORS,
    MQLParseFirstHalf,
    PostProcessAndValidateQuery,
    parse_mql_query_body,
    populate_query_from_mql_context,
)
from snuba.query.parser.exceptions import ParsingException
from snuba.query.query_settings import HTTPQuerySettings, QuerySettings
from snuba.query.snql.parser import CustomProcessors, parse_snql_query_initial
...
def test_autogenerated(theinput: tuple[Query,QuerySettings | None,CustomProcessors | None], expected: Query | CompositeQuery[Entity]) -> None:
...
def test_autogenerated_invalid(theinput: tuple[Query,QuerySettings | None,CustomProcessors | None], expected_error: Type[Exception]) -> None:
    with pytest.raises(expected_error):
        timer = Timer("mql_pipeline")
        PostProcessAndValidateQuery().execute(
            QueryPipelineResult(
                data=theinput,
                error=None,
                query_settings=HTTPQuerySettings(),
