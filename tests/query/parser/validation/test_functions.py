...
def test_invalid_function_name(expression: FunctionCall, should_raise: bool) -> None:
    ...
    fnames = [
        ...
        "post_process",
        "post_process_mql",
        "replace_time_condition",
    ]
    paths = [f"tests/query/parser/unit_tests/test_{fname}.py" for fname in fnames]
    for i in range(1, 2):  # len(astlogger[0])
        with open(
            os.path.abspath(paths[i - 1]),
            "w",
        ) as f:
            f.write(
                """
...
"""
            ...
            for trace in astlogger:
                if i >= len(trace):
                    continue
                qin, qout = trace[i]
                f.write(f"pytest.param({qin},{qout}),\n")
            f.write(
                """
]

@pytest.mark.parametrize("queryin, expected", test_cases)
def test_autogenerated(mql: str, queryin: Query, expected: Query) -> None:
    actual = """
            )
            f.write(f"{fnames[i-1]}(queryin)\n")
