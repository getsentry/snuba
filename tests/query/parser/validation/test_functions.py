...
def kylewrite() -> None:
    ...
    with open(
        os.path.abspath("tests/query/parser/unit_tests/test_mql_parse_first_half.py"),
        "w",
    ) as f:
        f.write(
            """
...
def test_autogenerated(theinput: tuple[str, Dataset, dict[str, Any], QuerySettings | None], expected: Query) -> None:
    body, dataset, mql_context_dict, settings = theinput
    actual = MQLParseFirstHalf().execute(
...
def test_autogenerated_invalid(theinput: tuple[str, Dataset, dict[str, Any], QuerySettings | None], expected_error: Exception) -> None:
    body, dataset, mql_context_dict, settings = theinput
    actual = MQLParseFirstHalf().execute(
        # query_settings and timer are dummy and dont matter
        QueryPipelineResult(
