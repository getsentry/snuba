...
test_cases = [
        ...
        Query(
            ...
            selected_columns=[
                ...
                SelectedExpression(
                    ...
                    f.arrayJoin(column("exception_stacks"), alias="arrayJoin(exception_stacks)"),
                ),
            ],
            array_join=None,
            condition=and_cond(
    ...
    pytest.param(
        ...
        Query(
            ...
            having=f.notEquals(or_cond(literal(1), literal(1), literal(1), literal(1)), literal(0)),
            order_by=None,
            limitby=None,
            limit=10,
            offset=0,
...
def test_parse_failure_does_not_log_query_body() -> None:
    body = "MATCH (events) SELECT count() WHERE user_email = 'secret@example.com'"
    with (
        mock.patch(
            "snuba.query.snql.parser.SnQLVisitor",
            side_effect=ParsingException("invalid query"),
        ),
