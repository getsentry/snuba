...
# SELECT (avg(t.c) AS f) OFFSET 0 FROM (SELECT (t.c AS _snuba_c1), (f(t.c2) AS _snuba_f1) GROUPBY t.c4 ORDER BY t.c ASC ARRAY JOIN col WHERE equals(c4, $S) HAVING equals(c6, $S) LIMIT 100 BY c8 LIMIT 150 OFFSET 0 FROM Entity(events) SAMPLE 0.5)
SIMPLE_ANONYMIZED = (
    "SELECT (t.c AS _snuba_c1), (f(t.c2) AS _snuba_f1) GROUPBY t.c4 ORDER BY t.c ASC"
    " ARRAY JOIN col WHERE equals(c4, $S) HAVING equals(c6, $S)"
    " LIMIT 100 BY c8 LIMIT 150 OFFSET 0 FROM Entity(events) SAMPLE 0.5"
)
...
TEST_JOIN = [
    ...
    pytest.param(
        ...
        (
            "SELECT (avg(t.c) AS f) OFFSET 0 FROM (SELECT (t.c AS _snuba_c1), (f(t.c2) AS _snuba_f1) GROUPBY t.c4 ORDER BY t.c ASC"
            " ARRAY JOIN col WHERE equals(c4, $S) HAVING equals(c6, $S) LIMIT 100"
            " BY c8 LIMIT 150 OFFSET 0 FROM Entity(events) SAMPLE 0.5)"
        ),
        id="Nested Query",
    ),
    pytest.param(
        ...
        (
            "SELECT (ev.c AS _snuba_c1), (f(ev.c2) AS _snuba_f1) OFFSET 0 FROM LEFT ev, Entity(events)"
            " TYPE JoinType.INNER RIGHT gr, Entity(groupedmessage) ON ev.group_id gr.id"
        ),
        id="Basic Join",
    ),
    pytest.param(
        ...
        (
            "SELECT t.c OFFSET 0 FROM (SELECT t.c OFFSET 0 FROM (SELECT (avg(t.c) AS f) OFFSET 0 FROM"
            " (SELECT (simple_t.simple_c AS _snuba_simple) OFFSET 0 FROM Entity(events) SAMPLE 0.5)))"
        ),
...
def test_query_formatter(
    ...
    formatted_query_anonymized = format_query_anonymized(query)
