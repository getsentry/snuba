...
class TestApi(BaseTest):
    ...
    def test_conditions(self):
        ...
        assert set([d['issue'] for d in result['data']]) == set(
    ...
    def test_prewhere_conditions(self):
        ...
        assert (
            "PREWHERE project_id IN (1) AND positionCaseInsensitive(message, 'abc') != 0"
            in result['sql']
        )
    ...
    def test_nullable_datetime_columns(self):
        ...
        query = {'project': 1, 'selected_columns': ['received']}
    ...
    def test_row_stats(self):
        query = {'project': 1, 'selected_columns': ['platform']}
        result = json.loads(self.app.post('/query', data=json.dumps(query)).data)
