...
def test_provide_commit_strategy() -> None:
    ...
    message_p0 = Message(
        ...
        Tick(
            ...
            timestamps=Interval(
                epoch + timedelta(seconds=1), epoch + timedelta(seconds=5)
            ),
        ),
        epoch,
        2,
    ...
    strategy.submit(message_p0)
    assert next_step.submit.call_args_list == [
        mock.call(make_message_for_next_step(message_p0, False))
    ]

    next_step.reset_mock()

    # Partition 1, same timestamp -> now we commit the offset
    message_p1 = Message(
        partition,
        2,
        Tick(
    ...
    strategy.submit(message_p1)

    assert next_step.submit.call_args_list == [
        mock.call(make_message_for_next_step(message_p1, True))
    ]
...
def test_tick_buffer_with_commit_strategy() -> None:
    ...
    # Message in partition 1, submitted to next step
    message_1_0 = Message(
        commit_log_partition,
        6,
        Tick(
