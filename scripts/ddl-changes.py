...
def _main() -> None:
    ...
    if diff_result.returncode != 0:
        ...
        for line in lines:
            ...
            print(f"-- forward migration {migration_group} :" f" {migration_id}")
            runner.run_migration(migration_key, dry_run=True)
            print(f"-- end forward migration {migration_group} :" f" {migration_id}")

            print("\n\n\n")
            migration_key = MigrationKey(migration_group, migration_id)
            print(f"-- backward migration {migration_group} :" f" {migration_id}")
            runner.reverse_migration(migration_key, dry_run=True)
            print(f"-- end backward migration {migration_group} :" f" {migration_id}")
