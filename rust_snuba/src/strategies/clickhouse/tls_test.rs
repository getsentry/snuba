...
async fn run_tls_request(verify: bool) -> Result<reqwest::StatusCode, String> {
    ...
    // Pre-cache the load-balancing runtime config so `ClickhouseClient::send`
    // does not reach into Python, keeping the test hermetic (no need for the
    // SNUBA_TEST_PYTHON* harness env vars).
    patch_str_config_for_test(
        &format!("clickhouse_load_balancing:{TLS_TEST_STORAGE}"),
        Some("in_order"),
    );
    patch_str_config_for_test(
        &format!("clickhouse_load_balancing_first_offset:{TLS_TEST_STORAGE}"),
        None,
    );
