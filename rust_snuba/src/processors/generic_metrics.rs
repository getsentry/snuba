...
use crate::{KafkaMessageMetadata, types::{InsertBatch, RowData}};

use super::utils::{default_retention_days, DEFAULT_RETENTION_DAYS};
...
fn generate_timeseries_id(
    ...
    project_id: u64,
    metric_id: u64,
    tags: &BTreeMap<String, String>) -> u32
{
...
impl TryFrom<FromGenericMetricsMessage> for CountersRawRow {
    ...
    fn try_from(from: FromGenericMetricsMessage) -> anyhow::Result<CountersRawRow> {
        ...
        let count_value = match from.value {
            ...
            _ => return Err(anyhow::anyhow!("Unsupported values provided for counter metric type")),
        };
...
pub fn process_counter_message(
    ...
    let sentry_received_timestamp = DateTime::from_timestamp(msg.sentry_received_timestamp.try_into().unwrap(), 0);
...
impl TryFrom<FromGenericMetricsMessage> for SetsRawRow {
    ...
    fn try_from(from: FromGenericMetricsMessage) -> anyhow::Result<SetsRawRow> {
        ...
        let set_values = match from.value {
            ...
            _ => return Err(anyhow::anyhow!("Unsupported values provided for gauge metric type")),
        };
...
impl TryFrom<FromGenericMetricsMessage> for DistributionsRawRow {
    ...
    fn try_from(from: FromGenericMetricsMessage) -> anyhow::Result<DistributionsRawRow> {
        ...
        let distribution_values = match from.value {
            ...
            MetricValue::SetOrDistribution(values) => values.into_iter().map(|v| v as f64).collect(),
            _ => return Err(anyhow::anyhow!("Unsupported values provided for distribution metric type")),
        };
...
impl TryFrom<FromGenericMetricsMessage> for GaugesRawRow {
    ...
    fn try_from(from: FromGenericMetricsMessage) -> anyhow::Result<GaugesRawRow> {
        ...
        match from.value {
            MetricValue::Gauge { last, count, max, min, sum} => {
...
mod tests {
    ...
    #[cfg(test)]
    fn test_processor_with_payload(f: &ProcessingFunction, message: &str) -> Result<InsertBatch, anyhow::Error>{
    ...
    fn test_counter_processor_with_counter_message() {
        let result = test_processor_with_payload(&(process_counter_message as fn(rust_arroyo::backends::kafka::types::KafkaPayload, crate::types::KafkaMessageMetadata) -> std::result::Result<crate::types::InsertBatch, anyhow::Error>), DUMMY_COUNTER_MESSAGE);
        let expected_row = CountersRawRow {
            ...
            tags_raw_value: vec!["metric_e2e_spans_counter_v_VUW93LMS".to_string(), "production".to_string(), "init".to_string()],
            set_values: vec![],
            count_value: 1.0,
            ...
            partition: 0,
            offset: 1,
            granularities: vec![GRANULARITY_ONE_MINUTE, GRANULARITY_ONE_HOUR, GRANULARITY_ONE_DAY],
            decasecond_retention_days: None,
            min_retention_days: None,
            hr_retention_days: None,
            day_retention_days: None
        };
        assert_eq!(result.unwrap(), InsertBatch{
    ...
    fn test_counter_processor_with_set_message() {
        let result = test_processor_with_payload(&(process_counter_message as fn(rust_arroyo::backends::kafka::types::KafkaPayload, crate::types::KafkaMessageMetadata) -> std::result::Result<crate::types::InsertBatch, anyhow::Error>), DUMMY_SET_MESSAGE);
        assert_eq!(result.unwrap(), InsertBatch::default());
    }

    #[test]
    fn test_set_processor_with_set_message() {
        let result = test_processor_with_payload(&(process_set_message as fn(rust_arroyo::backends::kafka::types::KafkaPayload, crate::types::KafkaMessageMetadata) -> std::result::Result<crate::types::InsertBatch, anyhow::Error>), DUMMY_SET_MESSAGE);
        let expected_row = SetsRawRow {
            ...
            tags_indexed_value: vec![0; 3],
            tags_raw_value: vec!["metric_e2e_spans_set_v_VUW93LMS".to_string(), "production".to_string(), "errored".to_string()],
    ...
    fn test_set_processor_with_distribution_message() {
        let result = test_processor_with_payload(&(process_counter_message as fn(rust_arroyo::backends::kafka::types::KafkaPayload, crate::types::KafkaMessageMetadata) -> std::result::Result<crate::types::InsertBatch, anyhow::Error>), DUMMY_DISTRIBUTION_MESSAGE);
        assert_eq!(result.unwrap(), InsertBatch::skip());
    }

    #[test]
    fn test_distribution_processor_with_distribution_message() {
        let result = test_processor_with_payload(&(process_distribution_message as fn(rust_arroyo::backends::kafka::types::KafkaPayload, crate::types::KafkaMessageMetadata) -> std::result::Result<crate::types::InsertBatch, anyhow::Error>), DUMMY_DISTRIBUTION_MESSAGE);
        let expected_row = DistributionsRawRow {
            ...
            tags_key: vec![65690, 9223372036854776010, 9223372036854776017],
            tags_indexed_value: vec![0; 3],
            tags_raw_value: vec!["metric_e2e_spans_dist_v_VUW93LMS".to_string(), "production".to_string(), "healthy".to_string()],
            set_values: vec![],
            count_value: 0.0,
    ...
    fn test_distribution_aggregate_option() {
        let result = test_processor_with_payload(&(process_distribution_message as fn(rust_arroyo::backends::kafka::types::KafkaPayload, crate::types::KafkaMessageMetadata) -> std::result::Result<crate::types::InsertBatch, anyhow::Error>), DUMMY_DISTRIBUTION_MESSAGE_WITH_HIST_AGGREGATE_OPTION);
        let expected_row = DistributionsRawRow {
            use_case_id: "spans".to_string(),
            org_id: 1,
            project_id: 3,
    ...
    fn test_distribution_processor_with_gauge_message() {
        let result = test_processor_with_payload(&(process_distribution_message as fn(rust_arroyo::backends::kafka::types::KafkaPayload, crate::types::KafkaMessageMetadata) -> std::result::Result<crate::types::InsertBatch, anyhow::Error>), DUMMY_GAUGE_MESSAGE);
        assert_eq!(result.unwrap(), InsertBatch::skip());
    }

    #[test]
    fn test_gauge_processor_with_gauge_message() {
        let result = test_processor_with_payload(&(process_gauge_message as fn(rust_arroyo::backends::kafka::types::KafkaPayload, crate::types::KafkaMessageMetadata) -> std::result::Result<crate::types::InsertBatch, anyhow::Error>), DUMMY_GAUGE_MESSAGE);
        let expected_row = GaugesRawRow {
            ...
            tags_key: vec![65690, 9223372036854776010, 9223372036854776017],
            tags_indexed_value: vec![0; 3],
            tags_raw_value: vec!["metric_e2e_spans_gauge_v_VUW93LMS".to_string(), "production".to_string(), "init".to_string()],
            set_values: vec![],
            count_value: 0.0,
            distribution_values: vec![],
    ...
    fn test_gauge_processor_with_aggregate_option() {
        let result = test_processor_with_payload(&(process_gauge_message as fn(rust_arroyo::backends::kafka::types::KafkaPayload, crate::types::KafkaMessageMetadata) -> std::result::Result<crate::types::InsertBatch, anyhow::Error>), DUMMY_GAUGE_MESSAGE_WITH_TEN_SECOND_AGGREGATE_OPTION);
        let expected_row = GaugesRawRow {
            use_case_id: "spans".to_string(),
            org_id: 1,
            project_id: 3,
            ...
            partition: 0,
            offset: 1,
            granularities: vec![GRANULARITY_ONE_MINUTE, GRANULARITY_ONE_HOUR, GRANULARITY_ONE_DAY, GRANULARITY_TEN_SECONDS],
            decasecond_retention_days: None,
            min_retention_days: None,
            hr_retention_days: None,
            day_retention_days: None,
    ...
    fn test_gauge_processor_with_counter_message() {
        let result = test_processor_with_payload(&(process_gauge_message as fn(rust_arroyo::backends::kafka::types::KafkaPayload, crate::types::KafkaMessageMetadata) -> std::result::Result<crate::types::InsertBatch, anyhow::Error>), DUMMY_COUNTER_MESSAGE);
        assert_eq!(result.unwrap(), InsertBatch::skip());
    }
}
