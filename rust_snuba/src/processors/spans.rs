...
impl TryFrom<FromSpanMessage> for Span {
    ...
    fn try_from(from: FromSpanMessage) -> Result<Span, InvalidMessage> {
        ...
        Ok(Self {
            ...
            module: from.sentry_tags.module.unwrap_or_default().clone(),
            op: from.sentry_tags.op.unwrap_or_default().clone(),
            parent_span_id: from.parent_span_id,
            platform: from.sentry_tags.system.unwrap_or_default().clone(),
            profile_id: from.profile_id,
            project_id: from.project_id,
            retention_days: from.retention_days.unwrap_or(90),
            segment_id: from.segment_id,
            segment_name: from.sentry_tags.transaction.unwrap_or_default().clone(),
            sentry_tag_keys,
            sentry_tag_values,
