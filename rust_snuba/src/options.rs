use std::time::Duration;

use sentry_options::options;

pub struct LoadBalancingConfig {
    pub load_balancing: String,
    pub first_offset: Option<String>,
}

pub fn get_load_balancing_config(storage_name: &str) -> LoadBalancingConfig {
    // Both keys live in the `snuba` sentry-options namespace as dicts keyed by
    // storage name (migrated from the per-storage runtime config keys
    // `clickhouse_load_balancing[_first_offset]:<storage>`).
    let snuba_options = options("snuba").ok();

    let load_balancing = snuba_options
        .as_ref()
        .and_then(|o| o.get("clickhouse_load_balancing").ok())
        .and_then(|v| {
            v.get(storage_name)
                .and_then(|s| s.as_str())
                .map(String::from)
        })
        .unwrap_or_else(|| "in_order".to_string());

    let first_offset = snuba_options
        .as_ref()
        .and_then(|o| o.get("clickhouse_load_balancing_first_offset").ok())
        .and_then(|v| {
            v.get(storage_name)
                .and_then(|s| s.as_str())
                .map(String::from)
        });

    LoadBalancingConfig {
        load_balancing,
        first_offset,
    }
}

/// ClickHouse's compiled-in default for `max_insert_block_size`. We refuse to
/// apply any override below this to avoid silently shrinking blocks below what
/// the server would already produce on its own.
pub const CLICKHOUSE_DEFAULT_MAX_INSERT_BLOCK_SIZE: u64 = 1_048_449;

/// Returns Some(n) if `clickhouse_max_insert_block_size:<storage_name>` is set
/// to an integer >= ClickHouse's default (1_048_449); otherwise None. Values
/// below the default are rejected, since they wouldn't increase the block size
/// past what ClickHouse already does by default. Callers should append
/// `&max_insert_block_size=<n>` to the INSERT URL when Some.
pub fn get_max_insert_block_size(storage_name: &str) -> Option<u64> {
    options("snuba")
        .ok()
        .and_then(|o| o.get("clickhouse_max_insert_block_size").ok())
        .and_then(|v| v.get(storage_name).and_then(|n| n.as_u64()))
        .filter(|&n| n >= CLICKHOUSE_DEFAULT_MAX_INSERT_BLOCK_SIZE)
}

/// Deadline for one logical ClickHouse write, covering every retry attempt
/// `reqwest` makes internally.
///
/// `reqwest` applies no timeouts of its own, so a write whose connection has
/// been silently black-holed (a load balancer or NAT gateway dropping the flow
/// without sending a RST) blocks until the kernel gives up retransmitting —
/// roughly 15 minutes with the default `tcp_retries2`. This is what bounds that.
///
/// Cumulative, not per attempt. `reqwest`'s retry layer sits inside the service
/// stack while the deadline is applied above it, so the clock spans the whole
/// retry sequence and does not reset between attempts. A stalled connection
/// therefore consumes the entire budget without retrying, while fast failures
/// (a 5xx, a reset) leave nearly all of it available for the retries.
///
/// 300s matches the `--batch-write-timeout-ms` the deployments already set, and
/// keeps the 1.5x relationship with their 450s `--max-poll-interval-ms`. It also
/// clears a healthy INSERT by a wide margin — those can legitimately run up to
/// a minute, and cutting off real work would turn a merely slow shard into a
/// retry storm against it.
pub const DEFAULT_CLICKHOUSE_WRITE_TIMEOUT: Duration = Duration::from_secs(300);

/// Whole-write deadline for `storage_name`, overridable via the
/// `clickhouse_write_timeout_ms` dict in the `snuba` options namespace.
///
/// Read fresh on every write (like [`get_load_balancing_config`]), so retuning
/// it takes effect on a running consumer without a redeploy.
///
/// Named for the write rather than the request because it bounds the retry
/// sequence as a whole, not one HTTP attempt — reading it the other way would
/// invite setting it to a single attempt's worth and unwittingly cutting the
/// retries off. Absent or non-positive values fall back to
/// [`DEFAULT_CLICKHOUSE_WRITE_TIMEOUT`].
pub fn get_clickhouse_write_timeout(storage_name: &str) -> Duration {
    options("snuba")
        .ok()
        .and_then(|o| o.get("clickhouse_write_timeout_ms").ok())
        .and_then(|v| v.get(storage_name).and_then(|n| n.as_u64()))
        .filter(|&ms| ms > 0)
        .map(Duration::from_millis)
        .unwrap_or(DEFAULT_CLICKHOUSE_WRITE_TIMEOUT)
}

#[cfg(test)]
mod tests {
    use super::*;
    use sentry_options::testing::override_options;
    use serde_json::json;
    use std::sync::Once;

    static INIT: Once = Once::new();

    fn init_options() {
        INIT.call_once(|| crate::init_sentry_options().unwrap());
    }

    #[test]
    fn test_load_balancing_config_defaults() {
        init_options();
        let config = get_load_balancing_config("lb_defaults_test");
        assert_eq!(config.load_balancing, "in_order");
        assert_eq!(config.first_offset, None);
    }

    #[test]
    fn test_load_balancing_config_overrides() {
        init_options();
        let _guard = override_options(&[
            (
                "snuba",
                "clickhouse_load_balancing",
                json!({ "lb_overrides_test": "first_or_random" }),
            ),
            (
                "snuba",
                "clickhouse_load_balancing_first_offset",
                json!({ "lb_overrides_test": "1" }),
            ),
        ])
        .unwrap();

        let config = get_load_balancing_config("lb_overrides_test");
        assert_eq!(config.load_balancing, "first_or_random");
        assert_eq!(config.first_offset, Some("1".to_string()));
    }

    #[test]
    fn test_clickhouse_write_timeout_default() {
        init_options();
        assert_eq!(
            get_clickhouse_write_timeout("request_timeout_defaults_test"),
            DEFAULT_CLICKHOUSE_WRITE_TIMEOUT
        );
    }

    #[test]
    fn test_clickhouse_write_timeout_override() {
        init_options();
        let _guard = override_options(&[(
            "snuba",
            "clickhouse_write_timeout_ms",
            json!({ "request_timeout_overrides_test": 5_000 }),
        )])
        .unwrap();

        assert_eq!(
            get_clickhouse_write_timeout("request_timeout_overrides_test"),
            Duration::from_millis(5_000)
        );
        // A different storage keeps the default.
        assert_eq!(
            get_clickhouse_write_timeout("request_timeout_other_storage"),
            DEFAULT_CLICKHOUSE_WRITE_TIMEOUT
        );
    }

    #[test]
    fn test_clickhouse_write_timeout_rejects_zero() {
        init_options();
        let _guard = override_options(&[(
            "snuba",
            "clickhouse_write_timeout_ms",
            json!({ "request_timeout_zero_test": 0 }),
        )])
        .unwrap();

        // Zero would mean "no deadline" to nobody's benefit — it is the exact
        // unbounded-wait behavior this option exists to prevent.
        assert_eq!(
            get_clickhouse_write_timeout("request_timeout_zero_test"),
            DEFAULT_CLICKHOUSE_WRITE_TIMEOUT
        );
    }
}
