...
if [ "$1" = 'api' ]; then
  if [ "$#" -gt 1 ]; then
    ...
    set -- uwsgi --master --manage-script-name --wsgi-file snuba/wsgi.py --die-on-term "${@:2}"
  else
    ...
    set -- uwsgi --master --manage-script-name --wsgi-file snuba/wsgi.py --die-on-term $_default_args
  fi
  set -- "$@"
fi
